...
export class GeminiParser {
  ...
  /**
   * Parse text using Gemini to extract calendar event information
   */
  async parseEventInfo(text: string, context?: string): Promise<ParsedEventInfo> {
    ...
    const prompt = `You are an expert at extracting event information from social media posts and text. 
...
${context ? `Context: ${context}\n\n` : ''}Text to parse: ${text}`;
    ...
    try {
      ...
      throw new Error(`Failed to parse event info: ${error.message || error}`);
    }
  }
